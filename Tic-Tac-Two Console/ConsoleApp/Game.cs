...
namespace ConsoleApp
{
    public class Game
    {
        ...
        private SessionRepositoryJson sessionRepository = new SessionRepositoryJson();
        ...
        public Game(GameSession session)
        {
            ...
            {
                this.gameBrain = new Brain(session.GameConfig);
            }
            else
            {
                this.gameBrain = new Brain(session.GameConfig, session.GameState);
            }
        ...
        {
            int[][] formattedBoard = new int[gameBrain.board.GetLength(0)][];
            for (int i = 0; i < gameBrain.board.GetLength(0); i++)
            {
                formattedBoard[i] = new int[gameBrain.board.GetLength(1)];
                for (int j = 0; j < gameBrain.board.GetLength(1); j++)
                {
                    formattedBoard[i][j] = gameBrain.board[i, j];
                }
            }
            GameState gameToSave = new GameState()
            {
                ...
                ChipsLeft = gameBrain.chipsLeft,
                PlayerNumber = gameBrain.playerNumber,
                PlayersMoves = gameBrain.playersMoves,
                Player1Options = gameBrain.player1Options,
                Player2Options = gameBrain.player2Options,
                Win = gameBrain.win
            };
            sessionRepository.SaveGameState(gameToSave, session.Id, sessionName);
        }
    }
}
